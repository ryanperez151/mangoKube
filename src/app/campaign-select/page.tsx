              ...
              <article key={id} aria-labelledby={`${id}-title`}>
                <Panel className="flex h-full flex-col p-7">
                  ...
                  <dl className="mt-6 grid gap-4 border-y border-white/10 py-5">
                    <div>
                      <dt className="font-mono text-[10px] uppercase tracking-[0.17em] text-slate-400">Role fantasy</dt>
                      <dd className="mt-1 text-sm leading-6 text-slate-200">{role?.fantasy}</dd>
                    </div>
                    <div>
                      <dt className="font-mono text-[10px] uppercase tracking-[0.17em] text-slate-400">Primary mechanic</dt>
                      <dd className="mt-1 text-sm leading-6 text-slate-200">{role?.primaryMechanic}</dd>
                    </div>
                    <div>
                      <dt className="font-mono text-[10px] uppercase tracking-[0.17em] text-slate-400">Learning focus</dt>
                      <dd className="mt-1 text-sm leading-6 text-slate-200">{role?.learningFocus}</dd>
                    </div>
                  </dl>

                  <div className="mt-5 flex-1">
                    <h3 className="font-mono text-[10px] uppercase tracking-[0.17em] text-slate-400">Five-stage operation</h3>
                    <ol className="mt-3 grid gap-2">
                      {campaign.stages.map((stage, stageIndex) => (
                        <li key={stage.id} className="flex items-center gap-3 text-sm text-slate-300">
                          <span className="flex h-6 w-6 items-center justify-center border border-white/10 font-mono text-[10px] text-slate-400">
                            {stageIndex + 1}
                          </span>
                          {stage.title}
                        </li>
